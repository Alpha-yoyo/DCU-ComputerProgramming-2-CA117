...
class Student(object):
    ...
    def add_mark(self, module, mark):
        return self.marks.update({module: mark})

    def precision_mark(self):
        weighted_value = sum([self.mods[code].ects * self.marks[code] for code in sorted(self.mods.keys())])
        credit = sum([CA1_MODULES[code].ects for code in sorted(CA1_MODULES.keys())])
        return weighted_value / credit

    def passed(self):
        check = [value for value in self.marks.values() if value > 40]
        return len(check) == 9

    def passed_by_compensation(self):
        if self.precision_mark() > 45:
            if len([value for value in self.marks.values() if value >= 35]) == 9:
                return sum([self.mods[code].ects for code in sorted(self.mods.keys()) if self.marks[code] < 40]) <= 10
